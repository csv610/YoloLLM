...
class YoloFastSAM:
    """FastSAM wrapper for image segmentation and visualization."""

    def __init__(self, model_name: str = "small"):
        """
        Initialize YoloFastSAM with a specified model.

        Args:
            model_name: Model name key (default: "small")

        Examples:
            yolo_fastsam = YoloFastSAM("small")
            yolo_fastsam = YoloFastSAM("large")
        """
        ...
        self.model = FastSAM(str(model_path))
        self.image = None
    ...
    def _get_masks(self) -> list:
        ...
        if self.results is None:
            raise ValueError("No inference results. Call segment() first.")
...
def main():
    """Main function to run YoloFastSAM from command line."""
...
Examples:
  ...
  python yolo_fastsam.py -i image.jpg -m large
  python yolo_fastsam.py -i image.jpg -m small -o result.jpg -t 0.7
  python yolo_fastsam.py -i image.jpg -s -d my_masks
    ...
    parser.add_argument(
        ...
        type=str,
        default="small",
        help=f"Model to use (default: small). Options: {', '.join(AVAILABLE_MODELS.keys())}"
    )

    parser.add_argument(
        "-o", "--output",
        type=str,
        default="fastsam_segment.png",
        help="Path to save output image (default: fastsam_segment.png)"
    )
